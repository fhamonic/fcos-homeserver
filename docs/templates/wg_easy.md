...
The web interface is served over plain HTTP (`INSECURE=true`) on `http_port`, on the assumption that it is only reached from the local network or through the VPN itself. If you publish it through Caddy, restrict the route to the local network with a `custom` directive as in the example `metaconfig.yaml`. The WireGuard port has to be forwarded from the internet router to the server.
