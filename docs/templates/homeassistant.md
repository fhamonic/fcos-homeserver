...
!!! warning "Put the dongle on a USB extension cable"
    Run the dongle on a USB extension cable of about a metre, away from the machine, never plugged straight into the case. USB 3 ports, external disk enclosures and USB 3 hubs radiate broadband noise across the whole 2.4 GHz band, and on a mini PC they sit centimetres from the dongle's antenna.

    The failure this causes is easy to misread, because reception is unaffected. Sensors keep publishing at full link quality while every command, every interview and the first network formation fail. The log shows `MAC_CHANNEL_ACCESS_FAILURE` on each transmission, or "network commissioning timed out" on a freshly flashed coordinator. Moving the dongle to another port, a motherboard port included, does not help, and an energy scan can still report the channel as quiet, because the noise is near field. Only distance fixes it.
...
Zigbee2MQTT does not start while the dongle is unplugged: Podman retries every ten seconds, and Home Assistant and Mosquitto are unaffected. Plugging it in afterwards is enough.

Once everything runs, add the **MQTT** integration in Home Assistant (*Settings › Devices & services › Add integration*), with broker `localhost` and port `1883`, no credentials. Zigbee2MQTT announces its devices to Home Assistant through that broker: a *Zigbee2MQTT Bridge* device appears under the integration at once, and each paired device follows by itself. Nothing else is configured on the Home Assistant side; in particular, its *Add device* button does not pair Zigbee devices. Home Assistant also discovers devices on the local network (Chromecast, HomeKit, Sonos, ESPHome...) by multicast, which does not cross the pod's network; those are added by entering their address by hand. Zigbee devices are not concerned, since they arrive over MQTT.

Pairing happens in Zigbee2MQTT, whose frontend is reachable on the LAN at `http://<server>:<zigbee2mqtt_port>` even without a Caddy site. Before the first device, set the Zigbee channel under *Settings › Advanced* and restart from the frontend's menu: the default channel 11 overlaps Wi-Fi channel 1 and is the most crowded, 25 overlaps the least. Changing it later means re-pairing some devices. Then click *Permit join (All)*, which opens the network for about four minutes, and put the device in pairing mode with the gesture of its manual (typically holding its button five seconds until its LED blinks slowly). It shows up in the *Devices* tab, first as "interviewing", then with its model. The *Zigbee2MQTT Bridge* device in Home Assistant exposes the same *Permit join* switch. Install codes are not needed for ordinary Zigbee 3.0 devices. Pair each device within a metre of the dongle the first time, and factory reset anything that was paired to another hub, since it keeps that network until reset and never shows up otherwise.
...
!!! note "Coordinator firmware"
    The firmware a dongle ships with can be years behind the one the [Zigbee2MQTT adapter page](https://www.zigbee2mqtt.io/guide/adapters/) links for it, and the "Coordinator firmware version" log line gives its date. Flashing takes a minute from a laptop with the vendor's web flasher, and belongs before any pairing: a re-flash empties the coordinator, so the network has to be formed again. Forming is the one moment the dongle must transmit before any device can answer, so if it hangs on "network commissioning timed out", the placement above is the first thing to fix. If it still hangs, form the network on a workstation instead: run the Zigbee2MQTT image against the stick with a copy of the server's `configuration.yaml`, which carries the network keys, wait for "Zigbee2MQTT started!", stop it and move the stick back. The server then resumes the network rather than forming it.

!!! note "Reaching the two interfaces"
    Home Assistant trusts the `X-Forwarded-For` header of requests coming from any private address, as rootless Podman delivers Caddy's requests from the host's own address (or its gateway's). A machine on the LAN can thus spoof the client address Home Assistant records, which matters only for its login attempt bans. The Zigbee2MQTT frontend has no authentication at all, so anyone who reaches it can open the network for joining: list its Caddy site in [`caddy.private_sites`](caddy.md#caddyprivate_sites), or in [`caddy.protected_sites`](caddy.md#caddyprotected_sites) as in the example, and set an *Auth token* under its *Settings › Frontend* if the LAN is not trusted either.

!!! note "Do not add an `http:` block"
    The reverse proxy settings live in the store the template seeds, not in `configuration.yaml`. Since 2026.7 Home Assistant migrates an `http:` block into a five-minute trial that reverts unless confirmed in the web interface, then ignores the block and reports it as a repair issue. Without those settings every request through Caddy is answered `400 Bad Request`. Change them from *Settings › System › Network*.
